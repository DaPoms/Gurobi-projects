...
#include <filesystem> // for recursive file traversal
...
//Note this reading format differs from usual, it is: first 3 elements at start are candidate count, leconstraint count, and geconstraint count. Then there are the objective vals of each item, followed by capacity coefficients, then capacity limits, then demand coefficients, and finally demand limits
...
void runWarmGurobiMDMKP(t warmStartFunction, GRBEnv& env, ofstream& excel,  vector<problemSet>& caseNums, fs::path p) // this is the tabu tenure comparison variant of runWarmGurobiMDMKP
{
    
    for(auto caseNum : caseNums)
    {
        
        /* if(blockNum < 17) //// use this code to start at a point other than block 1
        {
            blockNum++;
            continue;
        }  */
...
//////////Model settings/////////////////////////(placed here due to gurobiAdditionalTime)
    ...
    model.set(GRB_DoubleParam_TimeLimit, gurobiAdditionalTime); //600 + whatever time is left from the warm start, this is the dynamic version
...
///////////////////////////////////////////////////////////////
        ...
        if(model.get(GRB_IntAttr_SolCount) > 0)
        {
            ...
            excel << p.filename() << "," <<  profit << "," << runTime.count() << "," << model.get(GRB_DoubleAttr_Runtime)  << "," << model.get(GRB_DoubleAttr_MIPGap) << endl; 
        }   
        else
        {
            excel << p.filename() << "," <<  -1 << "," << runTime.count() << "," << model.get(GRB_DoubleAttr_Runtime) << endl; 
        }
...
int main()
{
    ofstream excel("MDMKP_3600sSamehN1000Cases.csv"); //creates file for data to be put in, ios::app allows appending so .open doesn't overwrite
    ...
    for(const fs::directory_entry file : fs::recursive_directory_iterator(p))
    {
        ...
        vector<problemSet> caseSet; // holds single problem read from problem txt file (each txt contains 1 problem, thats why we have to do the recursive file reading) 
        formatCase(0, caseSet, problemSets); 
        runWarmGurobiMDMKP(tabuSearchMDMKP, env, excel, caseSet, file.path()); //you pass functions just by name
