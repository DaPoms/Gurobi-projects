   ...
   //         demandRequirement *= 0.8; 
    /////////////////////////
        ...
        */
        ...
        vector<BoolVar> x; //variable for if we include / not include item in knapsack
...
//////////////////// capacity constraint ///////////////
        ...
        for(int i{0}; i < caseNum.knapsackCapacityVals.size(); i++) 
        {
            ...
            model.AddLessOrEqual(capacityExpr, caseNum.knapsackCapacityVals[i]);
        }       
...
//////////////////// demand constraint ///////////////
        ...
        {
            LinearExpr demandConstr;
            for(int e{0}; e < caseNum.problemsByCase[0].size(); e++)
                demandConstr += x[e] * caseNum.problemsByCase[0][e].demandVal[i]; // REMINDER: FOR NON CASE 1, edit demandVAL[0
            model.AddGreaterOrEqual(demandConstr, caseNum.knapsackDemandRequirementVals[i]);
        }
        
////////////////////////////////////////////////////
    
        operations_research::sat::SatParameters p;
        p.set_max_time_in_seconds(timeLimitInSeconds);
        p.set_relative_gap_limit(0.0001);
        ...
        if(warmSol.size() != 0)
            for(int i{0}; i < caseNum.problemsByCase[0].size(); i++)
                if(warmSol[i]) //the overlap between both solutions it brought into warm start
                    ...
                    model.AddHint(x[i], false);
        const CpSolverResponse response = SolveWithParameters(model.Build(), p);   


        if(response.status() == CpSolverStatus::FEASIBLE || response.status() == CpSolverStatus::OPTIMAL) //if feasible
        {   
            excel << "B" << blockNum << "C" << caseCounter << "," <<  response.objective_value() << "," << response.wall_time() << "," << (std::abs(response.best_objective_bound() - response.objective_value())) / response.objective_value()  << endl; //obj will always be positive 
        }
        else // case of infeasible solution 
        {
            excel << "B" << blockNum << "C" << caseCounter << "," <<  -1 << "," << response.wall_time() / 1000 << endl; 
        }
