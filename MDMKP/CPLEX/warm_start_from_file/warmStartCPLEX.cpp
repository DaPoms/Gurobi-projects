...
void runSolverMDMKP(int timeLimitInSeconds, ofstream& excel, vector<problemSet>& caseNums, int caseCounter)
{
   ...
   //         demandRequirement *= 0.8; 
    /////////////////////////
        ...
        */
        ...
        IloNumExpr objective(env); //HUGE NOTE: CPLEX API follows the concept that all vars must be connected to the env from the start
        vector<IloBoolVar> x; //variable for if we include / not include item in knapsack
//////////////////// objective value definition ///////////////
        for(int i{0}; i < caseNum.problemsByCase[0].size(); i++) 
            x.push_back(IloBoolVar(env)); 
       
        for(int i{0}; i < caseNum.problemsByCase[0].size(); i++)
                objective += x[i] * caseNum.problemsByCase[0][i].value; 
...
//////////////////// capacity constraint ///////////////
        ...
        {
            IloNumExpr capacityExpr(env);  
            for(int e{0}; e < caseNum.problemsByCase[0].size(); e++)
                capacityExpr += x[e] * caseNum.problemsByCase[0][e].capacityVal[i]; // REMINDER: FOR NON CASE 1, edit demandVAL[0]
            model.add(capacityExpr <= caseNum.knapsackCapacityVals[i]);
            capacityExpr.end();
        }       
...
//////////////////// demand constraint ///////////////
        ...
        {
            IloNumExpr demandConstr(env);
            for(int e{0}; e < caseNum.problemsByCase[0].size(); e++)
                demandConstr += x[e] * caseNum.problemsByCase[0][e].demandVal[i]; // REMINDER: FOR NON CASE 1, edit demandVAL[0
            model.add(demandConstr >= caseNum.knapsackDemandRequirementVals[i]);
            demandConstr.end();
        }
...
////////////////////////////////////////////////////
        ...
        // warm start (if not provided, skips)
       vector<bool> warmSol = readWarmSols[caseCounter - 1][blockNum - 1];
        if(warmSol.size() != 0)
        {
            IloNumVarArray decisionVars(env);
            IloNumArray warmStartDecisionVals(env);
            for(auto elem : x)
                decisionVars.add(elem);

            for(int i{0}; i < caseNum.problemsByCase[0].size(); i++)
            {
                if(warmSol[i]) //the overlap between both solutions it brought into warm start
                    warmStartDecisionVals.add(1);
                else    
                    warmStartDecisionVals.add(0);
            }
            cplex.addMIPStart(decisionVars, warmStartDecisionVals);
        }
        else
            cout << "No warm start provided\n";

        objective.end();
        
        double startTime = cplex.getCplexTime();
        IloBool status = cplex.solve();
        double runTime = cplex.getCplexTime() - startTime;
        if(status)
            excel << "B" << blockNum << "C" << caseCounter << "," <<  cplex.getObjValue() << "," << runTime << "," << cplex.getMIPRelativeGap()  << endl; //obj will always be positive 
        else // case of infeasible solution 
            excel << "B" << blockNum << "C" << caseCounter << "," <<  -1 << "," << runTime << endl; 
        blockNum++;
    }
}
...
int main()
{
   ...
   vector<MDMKRawProblem> MDMKRawProblems;
    ...
    runSolverMDMKP(timeInSecondsPerProblem, excel, caseSet, caseNum);
