...
int main()
{
    ofstream excel("GARCIATEST.csv"); //creates file for data to be put in, ios::app allows appending so .open doesn't overwrite
...
/*     for(int i{0}; i <= 5; i++) //extracts cases 1-6 and runs gurobi on them
    ...
    vector<vector<int>> warmSols = getWarmSols("C:/Users/Pomer/Desktop/Gurobi projects/ct7_B1C3SingleTest", caseSet[0].problemsByCase[0].size());
